...
| K-Selection / clustering (home) | `app.py`                                                                  | K sweep, viz, CDTA map; uses `src/kmeans_numpy.py`; elbow heuristics match `**/api/cluster`** (perpendicular-distance `elbow_k`, Δ² inertia `elbow_k_kneedle`); writes cluster labels to session state |
