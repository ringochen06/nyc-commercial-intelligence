...
Run `streamlit run app.py` to open **K-Selection / clustering** (`app.py`). The **Ranking** UI is `pages/Ranking.py`: **hard SQL filters**, then **α·semantic + β·competition-penalty** (MinMax on the filtered rows; one **α** slider). Optional **Claude**.
...
2. **`python run_pipeline.py`** — `src/data_processing.py` cleans sources (including optional **Neighborhood Financial Health** / NFH CSV merged into `nbhd_clean` when the file is present); `src/feature_engineering.py` reads **raw** storefront filings (optional path in `run_pipeline.py`), spatially aggregates **storefront** counts by CDTA and primary business activity, merges **MOCEJ-style neighborhood profiles** and `nfh_*` columns on a normalized Community District key, then **imputes** remaining gaps in those merged numeric columns with **borough median, then citywide median** (dashboard-friendly proxy where a CDTA does not match a single profile row). It computes `commercial_activity_score` = `log1p`(`storefront_filing_count` × `avg_pedestrian`), `competitive_score` = `log1p`(`storefront_filing_count` / (`avg_pedestrian` + 1)), and `transit_activity_score` = `log1p`(`subway_station_count` × `avg_pedestrian`). Output: `data/processed/neighborhood_features_final.csv`. A healthy run ends with **no missing values** in that table; if any column still has NaN, investigate before shipping.
3. **Embeddings (for the app)** — `python -m src.embeddings` builds embeddings from neighborhood text profiles (including **every non-zero `act_*_storefront`** business-activity count, population proxies, and NFH fields where present — see `data/processed/README.md`); caches under `outputs/embeddings/`. **Default** (`EMBEDDING_BACKEND` unset): **auto** behavior — **OpenAI** `text-embedding-3-small` if `OPENAI_API_KEY` is set, else **local sentence-transformers**. `EMBEDDING_BACKEND=openai` forces OpenAI mode. `EMBEDDING_BACKEND=sentence_transformers` forces local only. Use `--force` after changing features or profile text so embeddings match the CSV.
...
- **Maximum** `competitive_score`, `shooting_incident_count`
...
- **One blend slider** sets **α ∈ [0, 1]** for **semantic similarity** (cosine similarity after MinMax on the filtered set). **β = 1 − α** applies to a **specific competitive score** recomputed from the selected storefront category (`__overall__` or `act_*_storefront`) as `log1p(count / (avg_pedestrian + 1))`.
...
- Build a matrix **cosine_sim, -specific_competitive_score** for those rows and apply `sklearn.preprocessing.MinMaxScaler` (column-wise, **0–1** on the filtered set). With a **single** row, scaling falls back to a neutral mid-score to avoid degenerate MinMax.
- **`blended_score = α·col0 + β·col1`**. Sort by `blended_score` descending. The table shows `semantic_similarity`, `specific_competitive_score`, and `blended_score`. Use the dataframe **download** control in the UI to export the ranking.
...
| **Ranking** (`pages/Ranking.py`)               | **Hard SQL filters** (including max caps for competition/crime) → **MinMax([semantic, -specific_competitive_score])** → **α·col0 + (1−α)·col1** (optional map + Claude + optional cluster columns). |
...
python -m src.embeddings    # use --force to refresh; default auto backend: OpenAI when OPENAI_API_KEY set, else sentence-transformers
streamlit run app.py        # optional: ANTHROPIC_API_KEY for Claude panel
```

Copy `.env.example` to `.env` and set API keys as needed: by default embeddings run in **auto** mode (`EMBEDDING_BACKEND` unset) — OpenAI when `OPENAI_API_KEY` exists, otherwise sentence-transformers. `ANTHROPIC_API_KEY` is for the optional Claude panel.
...
Includes `tests/test_kmeans.py` and `tests/test_feature_engineering.py`.
...
Endpoints accept optional **`vintage`** (for example `/api/feature-ranges?vintage=present`). Only **`present`** is implemented today (loads `data/processed/neighborhood_features_final.csv`). Other values are reserved for future multi-year snapshots.
